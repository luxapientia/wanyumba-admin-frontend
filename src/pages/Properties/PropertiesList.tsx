...
import { 
  ...
  fetchAllProperties 
} from '../../store/thunks/propertiesThunks.js';
import {
  setPage,
  setLimit,
  setFilters,
...
export default function PropertiesList({ viewMode }: PropertiesListProps) {
  ...
  // Set view mode when component mounts or viewMode prop changes
  useEffect(() => {
  ...
  }, [dispatch, viewMode, page, limit, sortBy, sortOrder, filters.propertyType, filters.listingType, filters.minPrice, filters.maxPrice, filters.minBedrooms, filters.status, search]);
  ...
  const handleApprove = async (property: Property) => {
    ...
    try {
      ...
      dispatch(updateProperty(result));
  ...
  const handleLimitChange = (newLimit: number) => {
    ...
    // setLimit already resets page to 1 in Redux
  };

  const handleSearch = () => {
    dispatch(setSearch(searchInput));
    // setSearch already resets page to 1 in Redux
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-purple-50/30 p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-purple-600 via-pink-600 to-rose-600 bg-clip-text text-transparent">
                {viewMode === 'pending' ? 'Pending Approval' : 'All Properties'}
              </h1>
              <p className="text-gray-600 mt-1">{total} properties {viewMode === 'pending' ? 'awaiting approval' : 'in the system'}</p>
            </div>
            <Button
          ...
          {/* Status Filters - Only show for 'all' view */}
          {viewMode === 'all' && (
            <div className="mt-6 flex items-center gap-2 flex-wrap">
              <Filter size={18} className="text-gray-500" />
              <span className="text-sm font-medium text-gray-700">Status:</span>
              {(['ALL', 'DRAFT', 'PENDING', 'ACTIVE', 'REJECTED', 'SOLD', 'RENTED'] as const).map((status) => {
                const statusValue = status === 'ALL' ? undefined : status;
                const isActive = filters.status === statusValue || (status === 'ALL' && !filters.status);
                return (
                  <Button
                    key={status}
                    onClick={() => {
                      dispatch(setFilters({ status: statusValue as any }));
                    }}
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  ...
                  dispatch(setSorting({ sortBy: sortField as any, sortOrder: order as 'asc' | 'desc' }));
                  // setSorting already resets page to 1 in Redux
                }}
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    dispatch(setFilters({ propertyType: e.target.value || undefined }));
                    // setFilters already resets page to 1 in Redux
                  }}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    dispatch(setFilters({ listingType: e.target.value || undefined }));
                    // setFilters already resets page to 1 in Redux
                  }}
                  className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                >
            ...
            >
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    dispatch(setFilters({ minPrice: e.target.value ? parseFloat(e.target.value) : undefined }));
                    // setFilters already resets page to 1 in Redux
                  }}
                  className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    dispatch(setFilters({ maxPrice: e.target.value ? parseFloat(e.target.value) : undefined }));
                    // setFilters already resets page to 1 in Redux
                  }}
                  className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                />
          ...
          <>
            ...
            <div className="flex items-center justify-between bg-white rounded-2xl shadow-sm border border-gray-200/50 p-4">
              ...
              <div className="flex items-center gap-2">
                <Button
                  onClick={() => dispatch(setDisplayViewMode('grid'))}
                ...
                <Button
                  onClick={() => dispatch(setDisplayViewMode('list'))}
            ...
            >
              {items.map((property, index) => (
                <PropertyCard
                  ...
                  onApprove={viewMode === 'pending' ? handleApprove : undefined}
                  onReject={viewMode === 'pending' ? handleReject : undefined}
