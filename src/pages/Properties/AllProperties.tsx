...
import { fetchAllProperties } from '../../store/thunks/propertiesThunks.js';
import {
  setAllProperties,
  setAllLoading,
  setAllPage,
...
import { rejectProperty } from '../../store/thunks/propertiesThunks.js';
...
export default function AllProperties() {
  ...
  const { 
    ...
    search,
    displayViewMode,
  } = useAppSelector((state: RootState) => state.properties.all);
  ...
  useEffect(() => {
    dispatch(setAllProperties({ properties: [], pagination: { total: 0, page: 1, limit: 20, pages: 0 } }));
  ...
  const confirmRejection = async () => {
    ...
    try {
      ...
      dispatch(updateAllProperty(result));
  ...
  const handlePageChange = (newPage: number) => {
    dispatch(setAllPage(newPage));
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const handleLimitChange = (newLimit: number) => {
    dispatch(setAllLimit(newLimit));
  };

  const handleSearch = () => {
    dispatch(setAllSearch(searchInput));
  };
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    dispatch(setAllFilters({ propertyType: e.target.value || undefined }));
                  }}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    dispatch(setAllFilters({ listingType: e.target.value || undefined }));
                  }}
                  className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                >
            ...
            >
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    dispatch(setAllFilters({ minPrice: e.target.value ? parseFloat(e.target.value) : undefined }));
                  }}
                  className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    dispatch(setAllFilters({ maxPrice: e.target.value ? parseFloat(e.target.value) : undefined }));
                  }}
                  className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                />
              </div>
          ...
          <>
            ...
            <div className="flex items-center justify-between bg-white rounded-2xl shadow-sm border border-gray-200/50 p-4">
              ...
              <div className="flex items-center gap-2">
                <Button
                  onClick={() => dispatch(setAllDisplayViewMode('grid'))}
                ...
                <Button
                  onClick={() => dispatch(setAllDisplayViewMode('list'))}
