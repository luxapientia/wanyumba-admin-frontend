...
import { fetchValuerProfiles } from '../../store/thunks/professionalProfilesThunks.js';
import {
  setPage,
  setLimit,
  setFilters,
  ...
  setSearch,
  clearFilters,
} from '../../store/slices/valuerProfilesSlice.js';
import Button from '../../components/UI/Button.js';
import { Pagination } from '../../components/UI/index.js';
import type { ValuerProfile } from '../../api/index.js';
...
export default function Valuers() {
  ...
  const { 
    ...
    sortOrder, 
    search,
  } = useAppSelector((state) => state.valuerProfiles);
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600 mt-2">Manage and review valuer professional profiles</p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => dispatch(setFilters({ propertyType: e.target.value || undefined }))}
                className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => dispatch(setFilters({ valuationMethod: e.target.value || undefined }))}
                className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-r from-gray-50 to-blue-50/30 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider cursor-pointer hover:bg-blue-100/50 transition-colors"
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-gray-500 font-medium">No valuer profiles found</p>
                      <p className="text-gray-400 text-sm">Try adjusting your filters or search</p>
                    </div>
                  </td>
                </tr>
              ) : (
                profiles.map((profile: ValuerProfile) => (
                  <motion.tr
                    key={profile.id}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  ...
                  >
                        ...
                        <div className="relative">
                          {profile.professionalPhoto ? (
                            <img
                              ...
                              alt={profile.professionalTitle || 'Valuer'}
                              className="w-10 h-10 rounded-full object-cover ring-2 ring-gray-200"
                            />
                          ) : (
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center ring-2 ring-gray-200">
                              <span className="text-white font-semibold text-sm">
                                {(profile.user?.firstName?.[0] || profile.user?.email?.[0] || 'V').toUpperCase()}
                              </span>
                            </div>
                          )}
                          {profile.isVerified && (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        {profile.propertyTypes && profile.propertyTypes.length > 0 ? (
                          profile.propertyTypes.slice(0, 2).map((type, idx) => (
                            <span
                              key={idx}
                              className="px-2 py-1 text-xs font-semibold rounded-md bg-blue-100 text-blue-700 border border-blue-200"
                            >
                              {type}
                            </span>
                          ))
                        ...
                        {profile.propertyTypes && profile.propertyTypes.length > 2 && (
                          <span className="text-xs text-gray-500">+{profile.propertyTypes.length - 2}</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Button
                        onClick={() => navigate(`/professional-profiles/valuers/${profile.id}`)}
                        size="sm"
                        variant="ghost"
